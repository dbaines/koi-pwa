...
(function(document, window, $) {
    ...
    var Lightbox = (Ornament.Components.Lightbox = {
      ...
      defaults: {
        ...
        callbacks: {
          beforeOpen: function() {
            ...
            if (
              !(
                $anchor.is("[data-lightbox-size]") &&
                $anchor.attr("data-lightbox-size") === "fullscreen"
              )
            ) {
          ...
          open: function() {
            ...
            $(mfp.container)
              .find(".mfp-arrow-left")
              .html(Ornament.icons["chevron-left"]);
            $(mfp.container)
              .find(".mfp-arrow-right")
              .html(Ornament.icons["chevron-right"]);
          },
          elementParse: function(item) {
            if (item.type === "ajax") {
              item.src =
                item.src + (item.src.indexOf("?") > -1 ? "&" : "?") + "lightbox=true";
            }
          },
      ...
      sizeLightboxToViewport: function(mfp) {
        if (Lightbox.scrollable) {
          ...
          var maxLightboxHeightInPixels =
            windowHeight - topOffset - bottomOffset - $lightboxHeader.outerHeight();
          if ($lightboxFooter.length) {
            maxLightboxHeightInPixels =
              maxLightboxHeightInPixels - $lightboxFooter.outerHeight();
          }
          ...
          if (Lightbox.shadowable) {
            ...
            $lightboxContent
              .off("scroll", Ornament.U.Shadowable.shadowScrollY)
              .on("scroll", Ornament.U.Shadowable.shadowScrollY);
          }
        }
      },
      ...
      bindLightboxLink: function($anchor) {
        ...
        if (
          $anchor.is("[" + Lightbox.selectors.basicSelector + "]") ||
          anchorType !== "inline"
        ) {
      ...
      init: function() {
        ...
        var $lightboxAnchors = $(
          "[" +
            Lightbox.selectors.anchorSelector +
            "],[" +
            Lightbox.selectors.basicSelector +
            "]"
        );
  ...
  $.rails.allowAction = function(element) {
    ...
    $buttons
      .append($("<div />").append($modalConfirm))
